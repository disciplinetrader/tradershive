-- =============================================================================
-- Phase 2 · Increment 3b — fix + gate championship reward payout.
-- Run as postgres. Rollback: I3b_rollback.sql.
--
-- Two problems in finalize_championship:
--   1. It inserted xp_transactions/coin_transactions rows directly but never
--      updated profiles.xp/coins/level — so championship rewards were LEDGERED
--      but never actually credited to a spendable balance. Now routed through
--      award_xp_coins (service-role, idempotent on (user,'championship',champ)),
--      which credits the profile AND writes the ledger.
--   2. No minimum-field guard: a 1-registrant championship could mint a Champion
--      title, XP/coins and rating. Championships have no `ranked` flag (they are
--      all competitive), so the eligibility rule is >= 2 distinct participants.
--      Below that, results are recorded with 0 awards / no title, and the reward
--      ledger, XP/coin credit, hall-of-fame and rating are skipped.
-- =============================================================================
...
AS $function$
DECLARE
  v public.championships%ROWTYPE; r RECORD;
  v_top10 UUID[]; v_winner UUID; v_runner UUID; v_third UUID; v_stats JSONB;
...
BEGIN
  ...
  SELECT count(DISTINCT user_id) INTO v_participant_count
    FROM public.championship_participants WHERE championship_id=_champ;
  v_eligible := v_participant_count >= 2;

  -- results (0 awards / no title when the field is too small to be competitive)
  ...
  SELECT championship_id, user_id, rank, pnl, r_multiple, win_rate, profit_factor, max_drawdown,
    ...
    CASE WHEN NOT v_eligible THEN 0 WHEN rank=1 THEN 5000 WHEN rank<=3 THEN 2500 WHEN rank<=10 THEN 1000 WHEN rank<=100 THEN 400 ELSE 100 END,
    CASE WHEN NOT v_eligible THEN 0 WHEN rank=1 THEN 2500 WHEN rank<=3 THEN 1200 WHEN rank<=10 THEN 500 WHEN rank<=100 THEN 200 ELSE 50 END,
    CASE WHEN NOT v_eligible THEN NULL WHEN rank=1 THEN 'Champion' WHEN rank<=3 THEN 'Podium' WHEN rank<=10 THEN 'Top 10' WHEN rank<=100 THEN 'Top 100' ELSE NULL END
  ...
  IF v_eligible THEN
    -- reward ledger + ACTUAL profile credit (award_xp_coins does both)
    ...
    FOR r IN SELECT * FROM public.championship_results WHERE championship_id=_champ LOOP
      ...
      PERFORM public.award_xp_coins(
        r.user_id, r.xp_awarded, r.coins_awarded, 'championship', _champ,
        'Championship rank #' || r.final_rank);
    END LOOP;
  ...
  UPDATE public.championships SET status='completed',
    winner_user_id = CASE WHEN v_eligible THEN v_winner ELSE NULL END,
    updated_at=now() WHERE id=_champ;
