-- =============================================================================
-- Phase 2 · Increment 3f ROLLBACK. Run as postgres.
-- Restores the I3a finalize_battle (strict host guard: a non-host cannot
-- finalize even after end_at). This re-opens B-1, so emergency use only.
-- Identical to I3a_finalize_battle_rewards.sql.
-- =============================================================================
...
BEGIN
 ...
 END LOOP;
 SELECT count(DISTINCT user_id) INTO v_participant_count FROM public.battle_participants WHERE battle_id = _battle_id;
 ...
 UPDATE public.profiles SET elo = GREATEST(0, COALESCE(elo, 1000) + v_elo_change),
 peak_elo = GREATEST(peak_elo, COALESCE(elo, 1000) + v_elo_change), battles_played = battles_played + 1,
 ...
 END IF;
 IF v_eligible THEN
 FOR r IN SELECT user_id, final_rank, xp_awarded, coins_awarded FROM public.battle_results WHERE battle_id = _battle_id LOOP
 PERFORM public.award_xp_coins(r.user_id, r.xp_awarded, r.coins_awarded, 'battle', _battle_id, 'Battle Arena rank #' || r.final_rank);
 END LOOP;
 END IF;
