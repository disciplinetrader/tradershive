...
function ReplayPropFirmPage() {
    ...
    return (<div className="mx-auto w-full max-w-[1400px] space-y-6 pb-[var(--gutter-lg)] animate-in fade-in duration-500">
      <PageHeader title="Prop Firm Challenges" description="Practise prop-style evaluation rules using a virtual TradersHIVE account." actions={<Button asChild className="gradient-primary text-primary-foreground rounded-xl shadow-elegant">
      ...
      <div className="space-y-6">
        ...
        {activeChallenge ? (<div className="grid gap-6 lg:grid-cols-3">
          ...
          </div>) : (<div className="flex min-h-[400px] items-center justify-center rounded-3xl border border-dashed border-border/60 bg-card/30 p-8 text-center">
        ...
        {challenges && challenges.length > 0 && (<div className="space-y-4">
...
function ActiveChallengeOverview({ challenge }) {
    const { active: activeSession, setActive } = useActivePropChallenge();
    ...
    return (<GlassCard className="overflow-hidden border-primary/20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border-b border-border/40 bg-primary/5">
        ...
        <div className="flex items-center gap-2">
          {!isLinked && (<Button onClick={() => setActive({ id: challenge.id, paper_account_id: challenge.paper_account_id })} className="gradient-primary text-primary-foreground rounded-xl shadow-lg shadow-primary/20">
      ...
      <div className="p-6">
        <div className="grid gap-6 sm:grid-cols-3">
          <StatBox label="Current Balance" value={formatCurrency(Number(challenge.current_equity), challenge.currency)} sub={`Start: ${formatCurrency(Number(challenge.starting_equity), challenge.currency)}`}/>
          <StatBox label="Equity" value={formatCurrency(Number(challenge.current_equity), challenge.currency)} sub="Real-time"/>
          <StatBox label="Profit / Loss" value={`${Number(challenge.current_equity) >= Number(challenge.starting_equity) ? "+" : ""}${formatCurrency(Number(challenge.current_equity) - Number(challenge.starting_equity), challenge.currency)}`} valueClassName={Number(challenge.current_equity) >= Number(challenge.starting_equity) ? "text-emerald-400" : "text-rose-400"} sub={`${((Number(challenge.current_equity) - Number(challenge.starting_equity)) / Number(challenge.starting_equity) * 100).toFixed(2)}%`}/>
        </div>
...
function ActiveChallengeQuickStats({ challenge }) {
    ...
    return (<GlassCard className="p-6 space-y-6">
      <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
        <Target className="h-4 w-4"/> Rule Compliance
      </h3>
      
      <div className="space-y-5">
        <RuleMeter label="Daily Loss Limit" value={`${challenge.max_daily_loss_pct}%`} desc={`${formatCurrency(dailyLossLimit, challenge.currency)} max`} pct={0} // Server-side would calculate this based on today's starting equity
    />
        <RuleMeter label="Max Drawdown" value={`${challenge.max_total_drawdown_pct}%`} desc={`${formatCurrency(totalDdLimit, challenge.currency)} max`} pct={(currentDd / challenge.max_total_drawdown_pct) * 100}/>
        <RuleMeter label="Trading Days" value={`${challenge.trading_days_used} / ${challenge.min_trading_days}`} desc="Minimum required" pct={(challenge.trading_days_used / challenge.min_trading_days) * 100}/>
      </div>
...
function ChallengeCard({ challenge }) {
    ...
    return (<GlassCard className="group hover:border-primary/40 transition-all p-4">
      <div className="flex items-start justify-between mb-3">
        <div className="p-2 rounded-lg bg-primary/10 text-primary group-hover:scale-110 transition-transform">
          <GraduationCap className="h-4 w-4"/>
        </div>
        <Badge variant={isPassed ? "secondary" : isFailed ? "destructive" : "outline"} className={cn("capitalize text-[10px]", isPassed && "bg-emerald-500/10 text-emerald-500 border-none")}>
          {status}
        </Badge>
      </div>
...
function StatBox({ label, value, sub, valueClassName }) {
...
function RuleMeter({ label, value, pct, desc }) {
    ...
    return (<div className="space-y-2">
      ...
      <Progress value={clampedPct} className="h-1.5" indicatorClassName={cn(isDanger ? "bg-rose-500" : isWarning ? "bg-amber-500" : "bg-primary")}/>
