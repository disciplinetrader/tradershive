...
import { abandonPropChallenge, deletePropChallenge, getPropChallenge, tickPropChallenge, } from "@/lib/prop-challenges.functions";
...
function ChallengeDetail() {
    ...
    const statusBadge = challenge.status === "active" ? <Badge variant="secondary">Active</Badge> :
    ...
    return (<div className="space-y-5">
      ...
      <PageHeader title={challenge.name} description={`${challenge.preset.replace(/_/g, " ")} · ${formatCurrency(Number(challenge.account_size), challenge.currency)} · ${challenge.leverage}:1`} actions={<div className="flex items-center gap-2">
            {statusBadge}
            {challenge.status === "active" && (<Button size="sm" className="bg-primary hover:bg-primary/90" onClick={() => {
                    setActive({ id: challenge.id, paper_account_id: challenge.paper_account_id });
                    toast.success(activeSession?.id === challenge.id
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {challenge.status === "active" && (<DropdownMenuItem onSelect={() => setConfirmAbandon(true)}>Abandon challenge</DropdownMenuItem>)}
      ...
      <GlassCard className="p-4">
        ...
        <div className="mt-3">
          <EquityCurve days={days} startingEquity={Number(challenge.starting_equity)} drawdownLimit={drawdownLimitAmt}/>
        </div>
      </GlassCard>

      <GlassCard className="p-4">
        <div className="mb-3 text-sm font-semibold">Daily log</div>
        {days.length === 0 ? (<div className="flex flex-col items-start gap-2 text-xs text-muted-foreground">
            <span>Start your first trading session — closed trades appear here automatically.</span>
            {challenge.status === "active" && (<Button size="sm" onClick={() => {
                    setActive({ id: challenge.id, paper_account_id: challenge.paper_account_id });
                    navigate({ to: "/trading" });
                }}>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {[...days].reverse().map((d) => (<tr key={d.day_date} className="border-t border-border/40">
