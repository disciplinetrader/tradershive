...
function BattleArenaHome() {
  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="Featured" icon={Flame} items={featured.data} empty="No featured battles right now." />
      <Section title="Live now" icon={Swords} items={live.data} empty="No battles running." pulse />
      <Section title="Upcoming" icon={Clock} items={upcoming.data} empty="No upcoming battles — be the first to create one." />
      <Section title="My battles" icon={Swords} items={mine.data} empty="You haven't joined any battles yet." />
      <Section title="Recent results" icon={History} items={history.data} empty="No completed battles yet." />
    </div>
  );
}

function Section({ title, icon: Icon, items, empty, pulse }: { title: string; icon: React.ComponentType<{ className?: string }>; items?: any[]; empty: string; pulse?: boolean }) {
  return (
