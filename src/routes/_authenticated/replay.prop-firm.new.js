...
import { PROP_PRESETS, listPropPresets } from "@/lib/prop-challenges/presets";
...
function NewChallengePage() {
    ...
    const [presetId, setPresetId] = useState("ftmo");
    ...
    const m = useMutation({
        mutationFn: (payload) => create({ data: payload }),
        onSuccess: (row) => {
      ...
      <GlassCard className="p-4">
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {presets.map((p) => {
            ...
            return (<button key={p.id} type="button" onClick={() => applyPreset(p.id)} className={`cursor-pointer rounded-lg border p-3 text-left transition-colors ${active ? "border-primary bg-primary/5" : "border-border/40 bg-background/40 hover:border-primary/40"}`}>
      ...
      <GlassCard className="p-4">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Field label="Linked paper account">
            <Select value={form.paper_account_id || "__none"} onValueChange={(v) => setForm({ ...form, paper_account_id: v === "__none" ? "" : v })}>
          ...
          <NumField label="Account size" value={form.account_size} step={1000} min={100} onChange={(v) => setForm({ ...form, account_size: v })}/>
          <Field label="Currency">
            <Input value={form.currency} maxLength={3} onChange={(e) => setForm({ ...form, currency: e.target.value.toUpperCase() })}/>
          </Field>
          <NumField label="Profit target %" value={form.profit_target_pct} step={0.5} min={0.1} onChange={(v) => setForm({ ...form, profit_target_pct: v })}/>
          <NumField label="Max daily loss %" value={form.max_daily_loss_pct} step={0.5} min={0.1} onChange={(v) => setForm({ ...form, max_daily_loss_pct: v })}/>
          <NumField label="Max overall drawdown %" value={form.max_total_drawdown_pct} step={0.5} min={0.1} onChange={(v) => setForm({ ...form, max_total_drawdown_pct: v })}/>
          <NumField label="Min trading days" value={form.min_trading_days} step={1} min={0} onChange={(v) => setForm({ ...form, min_trading_days: v })}/>
          <NumField label="Duration (days)" value={form.duration_days} step={1} min={1} onChange={(v) => setForm({ ...form, duration_days: v })}/>
          <NumField label="Leverage" value={form.leverage} step={1} min={1} onChange={(v) => setForm({ ...form, leverage: v })}/>
          <NumField label="Commission / lot" value={form.commission_per_lot} step={0.5} min={0} onChange={(v) => setForm({ ...form, commission_per_lot: v })}/>
          <Field label="Spread profile">
            <Select value={form.spread_profile} onValueChange={(v) => setForm({ ...form, spread_profile: v })}>
          ...
          <Field label="Slippage profile">
            <Select value={form.slippage_profile} onValueChange={(v) => setForm({ ...form, slippage_profile: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <ToggleField label="Weekend hold allowed" value={form.weekend_hold_allowed} onChange={(v) => setForm({ ...form, weekend_hold_allowed: v })}/>
          <ToggleField label="News trading allowed" value={form.news_trading_allowed} onChange={(v) => setForm({ ...form, news_trading_allowed: v })}/>
        </div>

        <div className="mt-4 flex items-center justify-end gap-2">
          <Button variant="ghost" onClick={() => navigate({ to: "/replay/prop-firm" })}>Cancel</Button>
          <Button disabled={m.isPending || !form.name.trim()} onClick={() => m.mutate({
            ...form,
            preset: presetId,
...
function NumField({ label, value, onChange, step = 1, min = 0 }) {
    return (<Field label={label}>
      <Input type="number" inputMode="decimal" step={step} min={min} value={value} onChange={(e) => {
...
function ToggleField({ label, value, onChange }) {
    return (<div className="flex items-center justify-between rounded-md border border-border/40 bg-background/40 p-3">
