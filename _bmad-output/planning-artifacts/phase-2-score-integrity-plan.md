...
**Next task: finish Increment 2 — settlement spine.**

1. **Kraken trusted price provider (D-6):** add a server-side price resolver `src/lib/market-data/scored-price.server.ts`. Crypto → Kraken public Ticker (`api.kraken.com/0/public/Ticker`), with a symbol→Kraken-pair map and a fetch timeout; fx/indices/metals → existing `twelveDataQuote` proxy. **Fail closed** for scored trades: if no trusted price, throw a typed `temporary-price-unavailable` error and do not settle. Do NOT touch `chart_closed_trades` (practice). Add unit tests + a Kraken reachability probe from the deployment before relying on it.
2. **`open_trade` RPC** (jsonb-whitelisted insert, server-set `user_id/status='open'/opened_at`, server-derived `entry_price` for market orders). Keep broker validation (`validateNewOrder`) in the TS server fn.
3. **Caller migration** (same increment, before any lockdown): route `closeTrade`/`partialCloseTrade` through the new RPCs via `supabaseAdmin`; server-fetch the close price (scored) via the resolver; migrate `openTrade`, `moveToBreakEven`, `resetAccount`, `createAccount`, `setTradeExits`+exit-leg fills, `use-sl-tp-monitor`, `use-risk-monitor` (stop-out), `chart-trading/persist.ts`. Delete the module-private `commitSettlement`.
4. **Table contract** `I2_contract.sql` (+ rollback): revoke scored-column INSERT/UPDATE/DELETE on `paper_trades`/`paper_accounts`/`account_statistics` and write on `position_history` from `authenticated`; keep user-owned columns (notes, tag_ids, cosmetic account fields). Rehearse authz + full app-flow + settlement invariants + 2-session concurrency, then rollback-verify.

Then continue automatically: battle host-immutability/attribution/settlement/ties+rewards (wire `finalize_battle` to `award_xp_coins`) → championship lifecycle+UTC+scoring+scheduler (wire `finalize_championship` payout) → prop authoritative eval+cron → journal privacy (S-2 token RPC) → caller-supplied-ID definers (N-15) → historical ingestion (N-13 zero-progress, pagination, timeouts, session-gaps) → cron lifecycle (battle-settlement/championship) → email noop/stuck → grants/default-privileges → tests/CI → reconciliation report (D-7, report only, no data writes).

**Rehearsal DB state at this marker:** Phase 1 containment + Increment 1 + Increment 2 core applied; no synthetic data; paper tables empty. **Production untouched.**
