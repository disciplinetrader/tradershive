...
**Next increment: Increment 2 — settlement spine (paper trading server authority).**

Exact next task:
1. Build a trusted server-side crypto price provider per D-6: **Kraken** primary (public ticker), fail-closed for scored trades when no trusted price; practice/`chart_closed_trades` unchanged. Inspect `src/lib/market-data/*` for the existing provider interface before adding Kraken.
2. Author `docs/migrations/remediation/phase2/I2_settlement.sql` (+ `I2_rollback.sql`): SECURITY DEFINER `open_trade` / `close_trade` / `partial_close_trade` (service_role-only), identity from `auth.uid()` passed by the server fn as a checked arg, account-ownership + battle/championship participation + window + symbol-eligibility checks, server-derived price, server-side P&L (`computePnl`/`clampRealizedPnl` in TS, invariants in SQL), atomic balance/equity/stats/position_history write, `status='open'` guard in the close UPDATE (double-close impossible), `FOR UPDATE` account+trade lock (concurrent-close safe).
3. Migrate callers in the SAME increment before locking tables: `openTrade`, `closeTrade`, `partialCloseTrade`, `moveToBreakEven`, `resetAccount`, `createAccount`, `setTradeExits` + exit-leg fills, `use-sl-tp-monitor`, `use-risk-monitor` (stop-out), `chart-trading/persist.ts`.
4. Then contract: `I2_contract.sql` revoking scored-column writes on `paper_trades`/`paper_accounts`/`account_statistics`/`position_history` from `authenticated`, keeping user-owned columns (notes, tag_ids, cosmetic account fields).
5. Rehearse: apply → authz + settlement invariant + concurrency (double-close) + price-authority + app-flow tests with synthetic users → rollback verify → re-apply.

Then continue automatically through: battle host/attribution/settlement/ties → championship lifecycle+UTC+scoring → prop authoritative eval → journal privacy → caller-supplied-ID definers → historical ingestion → cron lifecycle → email → grants/default-privileges → tests/CI → reconciliation report.

**Rehearsal DB state at this marker:** Increment 1 applied; Phase 1 containment applied; no synthetic data left; profiles/paper tables empty.
